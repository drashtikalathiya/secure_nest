...
export default function PlanCard({
  ...
  subscribeLabel = "Select Plan for Your Nest",
  ...
  return (
    ...
    >
      ...
      <div className="my-6">
        ...
        <span className="text-gray-400 text-sm"> {plan.period || "/ 30 days"}</span>
      </div>
      ...
      {showSubscribeButton ? (
        <button
          onClick={(e) => {
            ...
            if (typeof onSubscribe === "function") {
              onSubscribe(plan.price_id, plan);
            }
          }}
          disabled={subscribeLoading || subscribeDisabled}
          className={`w-full h-11 mt-auto rounded-md font-semibold transition text-sm disabled:cursor-not-allowed disabled:opacity-60
            ${
              selected
        ...
        >
          {subscribeLoading ? "Redirecting..." : subscribeLabel}
        </button>
      ) : null}
    </div>
  );
